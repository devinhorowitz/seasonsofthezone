...
SPELL_FILE = ("; generated by _tools/season.py from seasons_config.py - do not edit by hand\r\n"
              "[calendar]\r\ncustom = false\r\ndial = default\r\n\r\n"
              "[spell]\r\nseason = %s\r\nname = Summer frost\r\nfirst = %s\r\nlast = %s\r\n")


@case
def t_a_spell_brings_its_season_while_its_days_last():
    """play.bat writes a spell that brings a season into season_calendar.ltx. On its days
    the blend and the snow are that season's, and the greeting and MCM name the spell; an
    MCM pin still wins; the day after its last, the calendar decides again. A spell the
    file gets wrong is left out, with a line in the log."""
    spell = SPELL_FILE % ("winter", "2026-07-14", "2026-07-15")
    _, g = build(2026, 7, 15, calendar=spell)
    z = g.zzz_seasons_of_the_zone
    mix = z.season_mix()
    assert mix["winter"] == 1.0 and mix["summer"] == 0, dict(mix)
    assert abs(z.snow_factor() - 0.60) < 1e-9, z.snow_factor()
    _, g = build(2026, 7, 16, calendar=spell)
    mix = g.zzz_seasons_of_the_zone.season_mix()
    assert mix["summer"] == 1.0 and mix["winter"] == 0, "outlived its days: %s" % dict(mix)
    _, g = build(2026, 7, 15, calendar=spell, mcm={"seasons_zone/main/mode": "autumn"})
    mix = g.zzz_seasons_of_the_zone.season_mix()
    assert mix["autumn"] == 1.0, "a spell won over a pin: %s" % dict(mix)

    lua, g = build(2026, 7, 15, calendar=spell, mcm={"seasons_zone/main/blend_days": 0})
    lua.execute("""
    ...
    run, rows, announce = probe(lua, g, "actor_on_first_update", "calendar_rows",
                                "pda_announce")
    run()
    got = rows()
    assert got[2]["text"] == ("Summer frost brings winter until Jul 15 - the calendar reads "
                              "summer."), got[2]["text"]
    announce()
    assert sent == ["Winter in the Zone for now: Summer frost, until July 15, 2026."], sent
...
def t_the_date_reads_month_day_year():
    ...
    greeting. Month first, as the rest of the page's dates are; the form before 2.0 was
    "25 September 2026"."""
